...
其中 $n$ 为谐波阶数。因此：
- $H_1$（$f_p = 1$ kHz）的相位偏移：$\Delta\theta_1 \approx 41°$
- $H_2$（$2f_p = 2$ kHz）的相位偏移：$\Delta\theta_2 \approx 82°$
...
- **I 分量**（$\cos$）：$|H_2| \cos(82°) = 0.139 \cdot |H_2|$
- **Q 分量**（$\sin$）：$|H_2| \sin(82°) = 0.990 \cdot |H_2|$
