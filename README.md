...
通过**导频抖动（pilot dithering）+ Goertzel 数字锁相**实现对光电调制器工作点的闭环偏压控制，目标控制精度 1°，长期免疫光功率与射频功率波动。当前实现 MZM 的 quad / max / min 工作点控制，架构预留 DDMZM、DPMZM、DPQPSK、PM 扩展接口。
...
DAC 同时输出偏压和导频正弦波，两路在每个 DRDY 中断（15.6 µs）同步叠加写入。Goertzel 算法从 ADC 采集的光电流中精确提取一次谐波（H1）和二次谐波（H2），以其比值相对于 DC 功率归一化后作为控制误差，从而消除光功率和射频功率波动的影响。
...
| [spec-02-dsp-pipeline](docs/plan/spec-02-dsp-pipeline.md) | 🔄 进行中 | 导频生成 + Goertzel 谐波提取，端到端验收 |
| [spec-03-mzm-quad](docs/plan/spec-03-mzm-quad.md) | ⏳ 待实现 | MZM 象限闭环控制（PI + 粗扫描 + 锁定检测） |
| [spec-04-robustness](docs/plan/spec-04-robustness.md) | ⏳ 待实现 | max/min 工作点、鲁棒性、UART 整定接口 |
...
串口设备名：macOS → `/dev/tty.usbmodem103`，Linux → `/dev/ttyACM0`，Windows → `COMx`
...
| 类型 | 常量 | 状态 |
|------|------|------|
| 单臂 MZM | `MOD_TYPE_MZM` | ✅ 已实现（quad/max/min） |
...
- **谐波幅度在计算误差前必须除以 DC 功率归一化**，以免疫光功率和射频功率的波动
...
| MZM 闭环控制 | [docs/plan/spec-03-mzm-quad.md](docs/plan/spec-03-mzm-quad.md) | 象限控制策略、PI 整定、验收标准 |
