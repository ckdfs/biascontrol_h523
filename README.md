...
DAC 同时输出偏压和导频正弦波，两路在每个 DRDY 中断（15.6 µs）同步叠加写入。Goertzel 算法从 ADC 采集的光电流中提取一次谐波（H1）和二次谐波（H2），控制层以带符号谐波分量和 DC 归一化结果估计当前工作点相位，并结合标定得到的 `null / peak / quad+ / quad-` 分支锚点执行局部闭环控制。H2 通道还加入了专用数字滤波（coherent I/Q EMA + 低 DC 冻结），以降低弱二次谐波的波动。
...
| [spec-03-mzm-quad](docs/plan/spec-03-mzm-quad.md) | 🔄 进行中 | MZM 全范围工作点控制：标定、锚点起锁、分支内局部控制 |
...
| `stop` | 停止控制环，返回 IDLE |
| `status` | 打印当前状态、偏压电压、锁定情况与当前 calibration anchors |
