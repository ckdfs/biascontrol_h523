...
```
APP       → src/app/        状态机、配置管理、UART 命令接口
CONTROL   → src/control/    偏压控制环、调制器策略（策略模式）、PI 控制器
DSP       → src/dsp/        Goertzel 谐波提取、导频正弦波生成（纯数学）
DRIVER    → src/drivers/    ADS131M02、DAC8568、板级 GPIO
...
│   ├── app/        app_config.{h,c}  app_main.{h,c}  app_state.h  app_uart.{h,c}
│   ├── control/    ctrl_bias.{h,c}   ctrl_modulator.h
│   │               ctrl_modulator_mzm.{h,c}  ctrl_pid.{h,c}
│   ├── dsp/        dsp_goertzel.{h,c}  dsp_pilot_gen.{h,c}  dsp_types.h
│   └── drivers/    drv_ads131m02.{h,c}  drv_board.{h,c}
│                   drv_callbacks.c  drv_dac8568.{h,c}
├── cubemx/                         ← CubeMX 输出，禁止手动修改
│   ├── biascontrol_h523.ioc        ← 唯一受 git 追踪的 CubeMX 文件
│   ├── Core/Inc, Core/Src
│   └── Drivers/CMSIS, Drivers/STM32H5xx_HAL_Driver
├── test/                           ← 主机端单元测试（原生编译）
├── scripts/                        ← 宿主机辅助 Python 脚本（采集/绘图）
├── cmake/                          ← 交叉编译工具链文件
├── docs/plan/                      ← 开发规格文档
└── CMakeLists.txt
```
...
| Spec | 状态 | 内容 |
|------|------|------|
| [spec-00-hardware](docs/plan/completed/spec-00-hardware.md) | 参考文档 | 硬件引脚映射、信号链、CubeMX 配置、NVIC、DMA |
| [spec-01-bringup](docs/plan/completed/spec-01-bringup.md) | ✅ 完成 | 板级启动：DAC8568、ADS131M02、USART1 驱动验证 |
| [spec-02-dsp-pipeline](docs/plan/completed/spec-02-dsp-pipeline.md) | ✅ 完成 | 导频生成 + Goertzel 谐波提取 + Vπ 表征 |
| [spec-03-mzm-quad](docs/plan/completed/spec-03-mzm-quad.md) | ✅ 完成（阶段基线） | 上一阶段 MZM 全范围控制基线：DC-assisted QUAD 路径与 5 min 套测 |
| [spec-04-mzm-no-dc-5hz](docs/plan/completed/spec-04-mzm-no-dc-5hz.md) | ✅ 完成 | 5 Hz、双扫描、全工作点无 DC 主控制——2026-04-13 验证 |
| [spec-05-robustness](docs/plan/active/spec-05-robustness.md) | ⏳ 待实现 | 长期稳定性、扰动恢复、参数持久化与整定接口 |
| [spec-06-multi-modulator](docs/plan/active/spec-06-multi-modulator.md) | 🔮 未来 | DDMZM、DPMZM、DPQPSK、PM 多调制器支持 |
...
1. 新建 `src/control/ctrl_modulator_<name>.c` 和 `src/control/ctrl_modulator_<name>.h`
...
策略接口定义（`src/control/ctrl_modulator.h`）：
...
| 硬件参考 | [docs/plan/completed/spec-00-hardware.md](docs/plan/completed/spec-00-hardware.md) | 引脚映射、DMA 通道、NVIC、CubeMX |
| 板级启动记录 | [docs/plan/completed/spec-01-bringup.md](docs/plan/completed/spec-01-bringup.md) | 驱动验证过程、调试经验与教训 |
| DSP 流水线 | [docs/plan/completed/spec-02-dsp-pipeline.md](docs/plan/completed/spec-02-dsp-pipeline.md) | Goertzel、导频生成、ISR 集成、验收标准 |
| MZM 闭环控制基线 | [docs/plan/completed/spec-03-mzm-quad.md](docs/plan/completed/spec-03-mzm-quad.md) | 上一阶段 MZM 全范围控制基线与验证结果 |
| 5 Hz 无 DC 控制 | [docs/plan/completed/spec-04-mzm-no-dc-5hz.md](docs/plan/completed/spec-04-mzm-no-dc-5hz.md) | 双扫描、5 Hz、无 DC 主控制——2026-04-13 验证完成 |
| 鲁棒性与持久化 | [docs/plan/active/spec-05-robustness.md](docs/plan/active/spec-05-robustness.md) | 长期稳定性、UART 整定接口、参数持久化 |
| 多调制器扩展 | [docs/plan/active/spec-06-multi-modulator.md](docs/plan/active/spec-06-multi-modulator.md) | DDMZM / DPMZM / DPQPSK / PM 规划 |
...
```bash
python3 scripts/capture_lock_response.py --target quad --duration 300 --poll 1.0
python3 scripts/capture_lock_response.py --target custom --custom-deg 45 --duration 300 --poll 1.0
```
